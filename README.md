...
### 무사진 서빙을 왜 모델 3개로 쪼갰나 — "오차"가 아니라 "정의"가 틀렸다

처음엔 **모델 하나였다.** 181명 직접측정 프로필로 둘레 7개 + 길이 5개, 총 12개 타깃을 한 번에 학습했다. 지금 구성이 셋으로 갈라진 건 설계가 아니라 **검증에서 나온 결과**다.

#### ① 길이 5개를 3D 정확 정답과 대조했더니 무너졌다

| 항목 | 옛 모델 MAE | 옛 모델 **편향** | 새 모델 MAE | 새 모델 편향 |
|---|---:|---:|---:|---:|
...
**봐야 할 건 MAE가 아니라 편향(bias)이다.** `leg_length`는 MAE 11.613cm에 편향 −11.612cm다. 오차가 거의 **전부 한 방향**이라는 뜻이고, 이건 모델이 못 맞힌 게 아니라 **애초에 다른 값을 재고 있었다**는 신호다. 무작위로 틀리면 편향은 0에 가까워야 한다.

원인은 랜드마크 정의였다.

| 항목 | 옛 정의 (181 proxy) | 새 정의 (exact v2) |
|---|---|---|
...
"바닥에서 샅까지의 높이"와 "골반점에서 복사뼈까지의 길이"는 **처음부터 다른 양**이다. 그 차이가 −11.6cm로 고스란히 나왔다. 모델을 더 튜닝했다면 이 편향은 절대 안 잡혔다.

#### ② 그래서 길이만 떼어내 3D 데이터로 다시 학습했다

- 원천 4,545행 → 결측·무한 57행, 무효 3행 제외 → **4,485행** (여성 2,510 / 남성 1,975)
- `MultiOutputRegressor(HistGradientBoostingRegressor)`, seed 42, shuffled 5-fold CV
- 아티팩트에 **sha256을 기록**해 어느 파일이 서빙 중인지 추적 가능하게 했다

편향이 −0.001 수준으로 떨어졌다. 정의를 맞추자 남은 건 순수 예측 오차뿐이다.

#### ③ 그런데 181 모델을 버리지는 않았다

**둘레와 길이의 학습 모집단이 다르고, 두 집합은 `subject_id` 교집합이 0명이다.** 하나로 합칠 수 없다. 그래서 버리는 대신 **한 모델의 출력 일부만 서빙한다**는 사실을 아티팩트에 박았다.
...
`targets`에는 12개가 남아 있지만 **서빙되는 건 4개뿐**이다. 이걸 파일에 명시하지 않으면, 나중에 누군가 `metrics_181.json`을 열어 길이 지표를 현재 성능으로 인용하게 된다.

#### ④ 부가 둘레 3개를 또 따로 둔 이유

181 모델도 `thigh`·`calf`·`arm`을 뱉지만, 별도 둘레 모델(178행)이 **세 항목 전부** 더 낫다.
...
그래서 이 3개는 별도 모델 값을 서빙하고, 181 모델의 **같은 이름 출력은 인용 금지**로 문서에 못박았다. 같은 파일에 같은 컬럼명으로 두 값이 존재하는 상황이라, 규칙을 적어두지 않으면 반드시 섞인다.

> ⚠️ **남은 부채** — 이 부가 둘레 모델만 `scripts/`에 학습 스크립트가 없다. 아티팩트·지표·행별 예측(178행)은 서로 일치하지만 **재학습 경로가 끊겨 있다.** 재학습이 필요해지면 스크립트부터 복원해야 한다. 숨기지 않고 평가 문서에 명시해 뒀다.

#### ⑤ 비율 2개는 모델을 만들지 않았다

`thigh_calf_ratio`와 `torso_leg_ratio`는 서버에서 **나눗셈으로 계산한다.**

- **일관성** — 비율을 따로 예측하면 응답 안의 길이 값과 비율이 서로 안 맞는다. 사용자가 한 화면에서 둘 다 보는데 계산이 어긋나면 그 순간 신뢰를 잃는다.
- **예측이 되지도 않는다** — `torso_leg_ratio`는 R² 0.020으로 사실상 집단 평균이다. 별도 모델을 만들어도 상수에 가까운 값을 뱉는다.

#### 정리 — 최종 서빙 구성

| 출력 | 개수 | 모델 | 모집단 | 왜 분리했나 |
|---|---:|---|---|---|
| `chest` `waist` `hip` `shoulder` | 4 | `hist_gradient_boosting_181` | 직접측정 172행 | 둘레는 직접측정 계열만 가능 |
| `thigh` `calf` `arm` | 3 | `hist_gradient_boosting_circumference` | 직접측정 178행 | 세 항목 전부 181 모델보다 우수 |
| 길이 5개 | 5 | `hist_gradient_boosting_exact_lengths_v2` | 3D 4,485행 | **옛 정의의 −11.6cm 편향** |
| 비율 2개 | 2 | 모델 없음 (후처리 나눗셈) | — | 일관성 + R² 0.020 |

**이 구조에서 배운 것** — 모델 성능을 MAE 하나로만 봤다면 `leg_length`의 정의 오류는 끝까지 못 찾았다. 편향을 따로 본 것, 그리고 정답 데이터를 바꿔가며 대조한 것이 결정적이었다.

### 왜 HistGradientBoosting인가 — KNN이 MAE 1위였는데도

**먼저 밝히면, 단일 지표로는 KNN이 이겼다.** 그런데도 HGB를 서빙에 올렸다. 그 판단 과정이 이 파트의 핵심이다.

모델 7종에 **같은 시험지를 풀렸다.** SizeKorea 정제 CSV 5,092행을 학습 4,092 / 공통 test 1,000명으로 나누고(`random_state=42`, `test_set.csv`로 고정 저장), 입력은 `gender·height·weight` 3개, 출력은 치수 7개. Hugging Face 모델 3종은 Colab GPU에서 돌렸지만 **같은 `test_set.csv`를 그대로 받아 썼기 때문에** 분할이 다른 별도 평가가 아니다.

| 순위 | 모델 | 구분 | MAE (cm) | RMSE (cm) | R² | 1건 예측 (ms) |
|---:|---|---|---:|---:|---:|---:|
| 1 | `knn` | 기본 | **1.915** | 2.465 | 0.785 | **0.013** |
| 2 | **`hist_gradient_boosting`** (채택) | 기본 | 1.920 | **2.461** | **0.786** | 0.170 |
...
| 7 | `baseline` (평균만 반환) | 기본 | 4.497 | 5.593 | −0.001 | 0.0001 |

**판단 1 — MAE 0.005cm 차이는 의미가 없고, RMSE 차이는 의미가 있다.**
KNN이 MAE에서 앞선 폭은 **0.005cm**다. 줄자 눈금(0.1cm)보다도, 측정자 간 오차보다도 작다. 반면 HGB는 **RMSE와 R² 두 지표에서 1위**다. MAE가 같은데 RMSE가 낮다는 건 **크게 틀리는 케이스가 더 적다**는 뜻이다. 사이즈 추천에서 평균 오차 0.005cm를 아끼는 것보다, 한 사람에게 크게 빗나가지 않는 쪽이 낫다고 봤다.

**판단 2 — KNN이 잘 맞은 이유 자체가 위험 신호였다.**
"키·몸무게가 비슷하면 체형도 비슷하다"는 직관과 KNN의 동작이 정확히 겹친다. 그래서 학습 분포 안에서는 잘 맞는다. 문제는 서비스 사용자가 SizeKorea 표본 분포 밖(예: 극단 BMI)에 있을 때다. KNN은 **가장 가까운 이웃을 그대로 평균**해서 학습 데이터 범위를 벗어난 값을 만들지 못한다. HGB는 트리 앙상블이라 이 경우에도 잔차 구조를 따라간다.

**판단 3 — Hugging Face 모델 3종은 전부 기각.**
정확도 우위가 없는데(4~6위) 1건 예측이 KNN 대비 **265~1,500배** 느리고, GPU와 외부 패키지 의존성이 붙는다. `tabpfn_mix`는 학습만 261초인데, target마다 AutoGluon predictor를 새로 학습하는 구조라 데이터·타깃이 늘수록 더 나빠진다. 실시간 추천 API에 넣을 이유가 없다.

**판단 4 — `baseline`을 반드시 같이 돌렸다.**
`DummyRegressor(strategy="mean")`은 입력을 보지 않고 평균만 반환한다. R² −0.001로 설명력이 0이었고, 나머지 6개가 **입력을 실제로 학습하고 있다**는 근거가 됐다. 오차를 4.50cm → 1.92cm로 절반 이하로 줄인 것도 이 기준선이 있어야 말할 수 있다.

**판단 5 — 여기서 모델 교체를 멈췄다.**
baseline을 뺀 6개가 MAE 1.915~2.008cm, R² 0.768~0.786에 **전부 수렴했다.** 전체 폭이 0.09cm·0.018이다. 이건 모델의 한계가 아니라 **입력이 `성별·키·몸무게` 3개뿐이라는 한계**다. 그래서 더 좋은 tabular 모델을 찾는 대신 **입력을 늘리는 쪽**으로 방향을 틀었고, 그 결론이 아래 사진(VLM) 경로다.

> **인용 시 주의** — 이 벤치마크는 정제 CSV 5,092행 · 치수 7개 기준이다. 현재 서빙 모델(§ 위 표)은 이후 **직접측정 181명 / 3D 4,485행으로 데이터셋을 다시 잡고 14개 항목으로 계약을 바꾼** 것이라, 이 표의 1.9cm와 서빙 지표의 MAE를 같은 숫자로 섞어 인용하면 안 된다. 모델 **선정 근거**로만 쓴다.
...
<img src="docs/cozy-features.svg" width="100%" alt="COZY 핵심 기능">
