...
| 기능 | 옷장 | 추구미 | 오늘의 룩 | 스타일리스트 | 코디 평가 | 룩북·캘린더·공유옷장 | 신체치수·사이즈 |
| 입력 | 옷·전신 사진 | 온보딩 선택·수정 | 위치·옷장·체형·TPO | 자연어·사진 | 착장 사진 | 룩/날짜/멤버 | 키·몸무게·성별·(사진) |
| 출력 | 분리된 아이템 + 태그 | 추천 가중치 신호 | 코디 카드 | 코디 카드 (SSE) | 8개 축 점수·코멘트 | 저장·기록·멤버 공유 | 14개 치수·비율 |
| 비고 | 일괄 등록·해시태그·재색인 | 누적 | 조회가 곧 생성 트리거 | 3인 페르소나(미니멀·실용·실험) | 근거 없으면 축 제외·재정규화 | 공유옷장은 `confirmed`만 | 무사진→VLM 2단 구조 |
