...
sequenceDiagram
    ...
    A-->>F: status + measurement 7개
```

### 5.1 2단으로 나눈 이유

VLM이 일부 부위를 빠뜨려도 응답에 빈칸이 생기지 않게 하기 위해서다. VLM이 7개를 다
...
| 경로      | 모델                                      | 예측 부위                      |
| --------- | ----------------------------------------- | ------------------------------ |
| 사진 없음 | `hist_gradient_boosting` (scikit-learn) | 7개                            |
| 사진 있음 | Kimi K2.5 (OpenRouter)                    | 7개 (기본 모델 값 위에 덮어씀) |
...
| 파일                                        | 용도               | 묻는 부위 |
| ------------------------------------------- | ------------------ | --------- |
| `prompts/body_measurement_prompt_full.j2` | 서빙               | 7개       |
...
| 응답 스키마 통일                                                  | ④ ↔ ⑥ 완전 동일                     |
| 사진 경로 HTTP 종단                                               | 여·남 각 1건 성공, 7개 부위 전부 반환 |
